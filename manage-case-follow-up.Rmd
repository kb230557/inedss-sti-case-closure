...
if(file.exists(paste0(fax_dir, "Provider Follow Up/", Sys.Date(), "_providers-with-no-database-record.csv"))){
  ...
  send.mail(from = key_get("cch_auto_email"),
            ...
            cc = c(key_get("sti_supervisor_email"), key_get("smtp_user")),
            subject = "Biweekly Provider Follow Up Lists",
            body = "SECURELOCK: Please see attached lists of STI providers who need fax numbers entered into the STI Provider database.",
            html = TRUE,
            smtp = list(host.name = key_get("smtp_host"), 
                        user.name = key_get("smtp_user"),
                        passwd = key_get("Office365"), tls = TRUE),
            attach.files = c(paste0(fax_dir, "Provider Follow Up/", Sys.Date(), "_providers-with-no-database-record.csv"), paste0(fax_dir, "Provider Follow Up/", Sys.Date(), "_providers-with-missing-fax.csv")),
            authenticate = TRUE,
            send = TRUE) 
...
if(length(email_files) >= 1){
  ...
  send.mail(from = key_get("cch_auto_email"),
            ...
            cc = key_get("smtp_host"),
            subject = "SECURELOCK: STI Cases Requiring Follow Up",
            body = "The attached files contain information on patients missing required information in I-NEDSS. Please submit missing information.",
            html = TRUE,
            smtp = list(host.name = key_get("smtp_host"), 
                        user.name = key_get("smtp_user"),
                        passwd = key_get("Office365"), tls = TRUE),
